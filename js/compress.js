// LZString 壓縮庫 (MIT License) - https://github.com/pieroxy/lz-string
...
const CompressSync = {
  ...
  decompressImport(compressed, skipPhotos = false) {
    try {
      ...
      console.error('解壓縮錯誤:', e);
      return { success: false, message: '❌ 解壓縮失敗：' + e.message };
    }
  },
  ...
  showCompressImportModal() {
    ...
    `;
    document.body.appendChild(modal);
    document.getElementById('compress-import-text').focus();
  },

  // 處理壓縮匯入
  processCompressImport() {
    const textarea = document.getElementById('compress-import-text');
    ...
    const result = this.decompressImport(cleaned, false);
